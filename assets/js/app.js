...
window.generateInvoicePDF = function() {
  ...
  if(bt==='flat'){
    var procedure=''; var amount=0;
    if(centerVal==='__custom__'||!center){
      procedure=(document.getElementById('inv-custom-procedure')||{}).value||'';
      amount=parseFloat((document.getElementById('inv-custom-amount')||{}).value)||0;
    } else {
      var frSel=document.getElementById('inv-flat-rate-select');
      var opt=frSel?frSel.options[frSel.selectedIndex]:null;
      amount=opt&&opt.value?parseFloat(opt.getAttribute('data-amount'))||0:0;
      procedure=opt&&opt.value?opt.text.split(' — ')[0].split(' -- ')[0].trim():'';
    }
    if(!procedure||!amount){alert('Please enter a procedure and flat rate amount.');return;}
    _generateFlatRateInvoicePDF(location,date,provider,procedure,amount);
    return;
  }
...
window.populateFlatRateDropdown = function() {
  ...
  var centerSel=document.getElementById('inv-location-select');
  if(!sel) return;
  var centerId=centerSel?centerSel.value:'';
  var center=(window.surgeryCenters||surgeryCenters||[]).find(function(c){return c.id===centerId;});
...
function injectBillingToggle() { setBillingType('hourly'); }
...
function _renderFlatRateInfoCard() { _refreshFlatRatePanel(); }
...
window.setBillingType = function(type) {
  ...
  var hRate=document.getElementById('inv-hourly-rate-fields');
  var fRate=document.getElementById('inv-flat-rate-display');
  var title=document.getElementById('inv-rate-card-title');

  // Reset both buttons
  if(btnH){btnH.style.border='2px solid var(--border)';btnH.style.background='var(--surface)';btnH.style.color='var(--text-muted)';btnH.style.fontWeight='500';}
  if(btnF){btnF.style.border='2px solid var(--border)';btnF.style.background='var(--surface)';btnF.style.color='var(--text-muted)';btnF.style.fontWeight='500';}

  if(type==='flat'){
    // Highlight Flat Rate button
    if(btnF){btnF.style.border='2px solid var(--info)';btnF.style.background='var(--info-light)';btnF.style.color='var(--info)';btnF.style.fontWeight='600';}
    if(hFields) hFields.style.display='none';
    if(fFields) fFields.style.display='';
    if(hRate)   hRate.style.display='none';
    if(fRate)   fRate.style.display='';
    if(title)   title.textContent='Flat Rates';
    window._billingType='flat';
    _refreshFlatRatePanel();
    populateFlatRateDropdown();
  } else {
    // Highlight Hourly button
    if(btnH){btnH.style.border='2px solid var(--info)';btnH.style.background='var(--info-light)';btnH.style.color='var(--info)';btnH.style.fontWeight='600';}
    if(hFields) hFields.style.display='';
    if(fFields) fFields.style.display='none';
    if(hRate)   hRate.style.display='';
    if(fRate)   fRate.style.display='none';
    if(title)   title.textContent='Rate Information';
    window._billingType='hourly';
    calculateInvoice();
  }
}
function _refreshFlatRatePanel() {
  ...
  if(val==='__custom__'||(!center&&val!=='')){
    // Custom: show text/number inputs, hide dropdown
    if(knownDiv) knownDiv.style.display='none';
    if(customDiv) customDiv.style.display='';
    if(title) title.textContent='Custom Flat Rate';
    if(rowsEl) rowsEl.innerHTML='<div style="padding:16px;font-size:13px;color:var(--text-faint);font-style:italic;text-align:center">Enter procedure and rate below</div>';
  } else if(center){
    // Known center: show procedure dropdown, hide custom inputs
    if(knownDiv) knownDiv.style.display='';
    if(customDiv) customDiv.style.display='none';
    if(title) title.textContent='Flat Rates — '+center.name;
    if(!frs.length){
      if(rowsEl) rowsEl.innerHTML='<div style="padding:16px;font-size:13px;color:var(--text-faint);font-style:italic;text-align:center">No flat rates set.<br>Edit this center in Analytics to add.</div>';
    } else {
    ...
    if(customDiv) customDiv.style.display='none';
    if(title) title.textContent='Flat Rates';
    if(rowsEl) rowsEl.innerHTML='<div style="padding:16px;font-size:13px;color:var(--text-faint);font-style:italic;text-align:center">Select a surgery center to see flat rates</div>';
  }
}
window.onCustomFlatChange = function() {
  var proc=(document.getElementById('inv-custom-procedure')||{}).value||'';
  var amount=parseFloat((document.getElementById('inv-custom-amount')||{}).value)||0;
  _updateFlatSummary(proc.trim(),amount);
}
function _updateFlatSummary(procedure,amount) {
  ...
  if(amount>0&&procedure){
    ...
    if(summaryEl) summaryEl.innerHTML=
      '<div style="font-size:11px;color:var(--text-faint);margin-bottom:4px;text-transform:uppercase;letter-spacing:.5px;font-weight:600">Flat Rate</div>'
      +'<div style="font-size:15px;font-weight:500;color:var(--text)">'+procedure+'</div>'
      +'<div style="font-size:14px;color:var(--accent);font-weight:600;margin-top:2px">$'+amount.toFixed(2)+'</div>';
  } else {
