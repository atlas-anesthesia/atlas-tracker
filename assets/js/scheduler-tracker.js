...
(() => {
  ...
  window.renderSchedulerTracker = async function() {
    ...
    const COLS = '1fr 140px 130px 130px 60px';
    let html = `<div style="display:grid;grid-template-columns:${COLS};gap:10px;padding:12px 16px;background:var(--surface2);border-bottom:1px solid var(--border);font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.5px;color:var(--text-faint)">
      <span>Patient</span><span>Visit</span><span>Called</span><span>$100 Paid</span><span></span>
    </div>`;
    rows.forEach(e => {
      ...
      const calledPill = called
        ? `<button onclick="window._strToggleCalled('${e.id}', false)" style="background:#dcfce7;color:#166534;border:1px solid #86efac;font-size:11px;font-weight:700;padding:4px 10px;border-radius:11px;cursor:pointer;font-family:inherit">✓ Called</button>`
        : `<button onclick="window._strToggleCalled('${e.id}', true)" style="background:#fff;color:#64748b;border:1px dashed #cbd5e1;font-size:11px;font-weight:600;padding:4px 10px;border-radius:11px;cursor:pointer;font-family:inherit">○ Not yet</button>`;
      const paidPill = paid
      ...
      html += `<div style="display:grid;grid-template-columns:${COLS};gap:10px;padding:12px 16px;border-bottom:1px solid var(--border);align-items:center">
