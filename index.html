...
async function runDailyPaymentBackup() {
  try {
    ...
    if(snap.exists()) {
      await setDoc(doc(db,'atlas','payments_backup_'+today), { rows: snap.data().rows||[], backedUpAt: new Date().toISOString() });
...
function syncPaymentRowsFromCases() {
  ...
  finalized.forEach(c => {
    ...
    if(rowIdx !== -1) {
      ...
      _paymentRows.push({ id:uid(), caseId:c.caseId, name:c.caseId||'', worker:c.worker||'josh',
...
function renderPaymentRows() {
  ...
  if(!_paymentRows.length) {
    body.innerHTML = '<div style="padding:32px;text-align:center;color:var(--text-faint);font-size:13px">No cases yet — save a Pre-Op record to auto-populate cases here</div>';
...
window.openInvoiceModal = function(rowIdx) {
  ...
  if(prov)prov.value=(r?.worker||currentWorker||'josh')==='dev'?'Dr. Dev Murthy':'Josh Condado';
  if(r?.surgeryCenter){
...
window.downloadInvoiceModal = async function() {
  ...
  if(billingType==='flat'){
    ...
    window._invModalCalc={total:amt,billedStr:'Flat Rate',flat:true,proc};
    _generateFlatRateInvoicePDF(location,date,document.getElementById('inv-modal-provider')?.value||'',proc,amt);
    await savePDFRecord({id:uid(),invoiceNum,location,date,provider:document.getElementById('inv-modal-provider')?.value||'',total:amt,caseId,worker:currentWorker,emailed:false,savedAt:new Date().toISOString()});
    ...
    _generateFlatRateInvoicePDF(location,date,document.getElementById('inv-modal-provider')?.value||'','Anesthesia Services',calc.total);
    await savePDFRecord({id:uid(),invoiceNum,location,date,provider:document.getElementById('inv-modal-provider')?.value||'',total:calc.total,caseId,worker:currentWorker,emailed:false,savedAt:new Date().toISOString()});
...
window.sendInvoiceEmail = async function() {
  ...
  try {
    const res=await fetch('https://atlas-reminder.blue-disk-9b10.workers.dev/invoice',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({to:email,location,date,provider,total,invoiceNum,worker:currentWorker||'josh',html:invoiceHTML})});
...
// ── showTab hook ──────────────────────────────────────────────────────
const _origShowTabPayFinal = window.showTab;
window.showTab = function(tab, pushState=true) {
  _origShowTabPayFinal(tab, pushState);
